...
def main():
    ...
    # Set the default image folder to your rgb_left folder
    parser.add_argument('--img_folder', type=str, default='/home/archer/cerd_data/pour_water_01/rgb_left', help='Folder with input images')
    parser.add_argument('--out_folder', type=str, default='/home/archer/cerd_data/pour_water_01/hand', help='Output folder to save rendered results')
    ...
    # Use file types that match your images (e.g. png) 
    parser.add_argument('--file_type', nargs='+', default=['*.png', '*.jpg'], help='List of file extensions to consider')
    ...
    # Create main subfolders (we'll add timestamp subfolders later)
    joints_folder = os.path.join(args.out_folder, "joints_csv")
    orientation_folder = os.path.join(args.out_folder, "orientation_csv")
    visuals_folder = os.path.join(args.out_folder, "visuals")
    os.makedirs(joints_folder, exist_ok=True)
    os.makedirs(orientation_folder, exist_ok=True)
    os.makedirs(visuals_folder, exist_ok=True)

    # For npy outputs (joints and orientation)
    joints_npy_folder_base = os.path.join(args.out_folder, "joints_npy")
    orientation_npy_folder_base = os.path.join(args.out_folder, "orientation_npy")
    os.makedirs(joints_npy_folder_base, exist_ok=True)
    os.makedirs(orientation_npy_folder_base, exist_ok=True)
    
    # If saving meshes, create a base folder
    if args.save_mesh:
        meshes_folder_base = os.path.join(args.out_folder, "meshes")
        os.makedirs(meshes_folder_base, exist_ok=True)

    # Recursively get all demo images from the given folder
    img_paths = []
    for file_type in args.file_type:
        img_paths.extend(list(Path(args.img_folder).rglob(file_type)))
    img_paths = sorted(img_paths)
    print(f"Found {len(img_paths)} images to process.")

    # Process each image
    for img_path in img_paths:
        # Extract the timestamp from the parent folder name (e.g. "timestamp1740349022")
        timestamp = Path(img_path).parent.name
        ...
        for batch in dataloader:
            ...
            for n in range(batch_size_):
                ...
                # Create timestamp subfolder inside visuals
                visuals_subfolder = os.path.join(visuals_folder, timestamp)
                os.makedirs(visuals_subfolder, exist_ok=True)
                out_img_name = f'{img_fn}_{person_id}.png'
                cv2.imwrite(os.path.join(visuals_subfolder, out_img_name), 255 * final_img[:, :, ::-1])
                ...
                # Create timestamp subfolder for joints CSV
                joints_csv_subfolder = os.path.join(joints_folder, timestamp)
                os.makedirs(joints_csv_subfolder, exist_ok=True)
                joints_csv_path = os.path.join(joints_csv_subfolder, f'{img_fn}_{person_id}_joints.csv')
                ...
                # Save joints as .npy file in a timestamp subfolder
                joints_npy_folder = os.path.join(joints_npy_folder_base, timestamp)
                ...
                orientation_csv_subfolder = os.path.join(orientation_folder, timestamp)
                os.makedirs(orientation_csv_subfolder, exist_ok=True)
                orientation_csv_path = os.path.join(orientation_csv_subfolder, f'{img_fn}_{person_id}_orientation.csv')
                ...
                orientation_npy_folder = os.path.join(orientation_npy_folder_base, timestamp)
                ...
                # Optionally save MANO meshes if requested
                if args.save_mesh:
                    meshes_subfolder = os.path.join(meshes_folder_base, timestamp)
                    os.makedirs(meshes_subfolder, exist_ok=True)
                    tmesh = renderer.vertices_to_trimesh(verts, cam_t.copy(), LIGHT_BLUE, is_right=is_right_flag)
                    tmesh_name = f'{img_fn}_{person_id}.obj'
                    tmesh.export(os.path.join(meshes_subfolder, tmesh_name))
                    print(f"Saved MANO mesh to {os.path.join(meshes_subfolder, tmesh_name)}")
        ...
        if args.full_frame and len(all_verts) > 0:
            ...
            cv2.imwrite(os.path.join(visuals_subfolder, front_view_name), 255 * input_img_overlay[:, :, ::-1])
